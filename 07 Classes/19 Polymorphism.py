...
class UIControl(ABC): # Here we are defining an abstract class with an abstract method.
    @abstractmethod
    def draw(self): # This abstract method with the decorater "@abstractmethod" defines the common feature or method that must be implemented in the the class that derive from  "UIControl()".
        pass
...
# def draw(control): # Here we also have a funtion called draw, that takes a controle object and call the draw method in that object.
#     control.draw()
...
# It simply iterates over the list of controls and call the the "draw()" of each contol.
