...
class Stream: # Here we are defining a class with the comom fetures between the streams, like open and close.
    ...
    def open(self): # if we try to open a stream that is already opened, that an invalid operation. So we can raise an exception
        if self.opened:
...
# We dont't have multi-level inheritance
